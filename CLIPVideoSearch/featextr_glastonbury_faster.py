...
parser.add_argument('--start', type=int, default=480000)
...
def collate_fn(batch):
    ...
    frames = torch.stack([x for y in frames for x in y], dim=0)
    fns = [item[1] for item in batch]
    fns = [x for y in fns for x in y]
    return frames, fns




class VideoFrameReader(Dataset):
    def __init__(self, video_list, preprocess=None):
    ...
    def __getitem__(self, idx):
        fn = self.list[idx]
        frames = self.extract_frames(fn)
        frames = [self.preprocess(frame) for frame in frames]
        return frames, [fn] * len(frames)


    def extract_frames(self,fn):
        # Open the video file
        # print(fn)
...
feat_fn_new = f'{args.feat_dir}feats_start{args.start}_end{args.end}_remaining.pkl'
...
print (f'Loading Dataloader for {len(video_list_to_process)} videos')
dataset = VideoFrameReader(video_list_to_process, preprocess=preprocess)
dataloder = DataLoader(dataset, batch_size = 8, drop_last=False, shuffle=False, num_workers=8, pin_memory=True, collate_fn=collate_fn)

tic = time.time()

for ii, (video_frames, fns) in tqdm(enumerate (dataloder), total= len(dataloder), ascii=True) :
   
    video_frames = video_frames.to(args.device)
    ...
    #     batch_preprocessed = torch.stack([preprocess(frame) for frame in batch_frames]).to(args.device) # 25, 3 224 224
    ...
    if ii%10==0:
        ...
        pickle_save(save_data, feat_fn_new)
...
# python featextr_glastonbury_faster.py --start 480000 --end 600000  --device 'cuda:3'  # feat7 