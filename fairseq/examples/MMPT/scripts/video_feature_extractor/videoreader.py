...
class VideoLoader(Dataset):
    ...
    def _run(self, cmd, output_file=None):
        out, _ = (
...
class VGGSound_VideoLoader(VideoLoader):
    """modified from how2's video_feature_extractor."""
    def __init__(
        self,
        done_videos = None,
        ...
        **kwargs
        ...
        if video_dict is None:
            raise ValueError("csv and video_dict cannot be both None.") 
        self.csv = pd.DataFrame.from_dict(video_dict)
        self.csv = self.csv[~self.csv.video_path.isin(done_videos)]
        # self.csv = self.csv.iloc[6356:]
        self.centercrop = centercrop
        self.size = size
        self.framerate = framerate
        self.hflip = hflip
...
class Glastonbury_VideoLoader(VideoLoader):
    """modified from how2's video_feature_extractor."""
    def __init__(
        self,
        dataset_part=None,
        ...
        **kwargs
        ...
        if video_dict is None:
            raise ValueError("csv and video_dict cannot be both None.") 
        self.csv = pd.DataFrame.from_dict(video_dict)
        self.csv = self.csv[self.csv['video_path'].apply(lambda x:x[-3:] != 'csv')]
        
        if dataset_part != 10:
            self.csv = self.csv[(dataset_part-1) * 100000 : dataset_part*100000]
        else:
            self.csv = self.csv[(dataset_part-1) * 100000 : ]
        self.csv = self.csv[~self.csv.video_path.isin(done_videos)]
        # self.csv = self.csv.iloc[6356:]
        self.centercrop = centercrop
        self.size = size
...
class Glastonbury_VideoLoader2(Glastonbury_VideoLoader):
    """modified from how2's video_feature_extractor."""
    def __init__(
        self,
        video_list=None,
        ...
        **kwargs
        ...
        self.video_list = [x for x in self.video_list if x not in done_videos]
        self.centercrop = centercrop
        self.size = size
        self.framerate = framerate
        self.hflip = hflip
    ...
    def __getitem__(self, idx):
        ...
        return self._decode(video_path)
